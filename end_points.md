...
| Method | Endpoint | Auth | Notes |
|--------|----------|------|-------|
| `GET` | `/api/privacy-policy` | No | Read the current privacy policy; returns an empty string until the admin sets it |
| `PUT` | `/api/admin/privacy-policy` | Yes (`ADMIN`) | Create or replace the privacy policy using `{ "content": "..." }` |
